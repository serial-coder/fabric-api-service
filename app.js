const CHANNEL_ID = process.env.CHANNEL_ID || 'common'
const CHAINCODE_ID = process.env.CHAINCODE_ID || 'example02'
...
const ChaincodeExample02Service = require('./service/chaincode-example02-service')
...
app.use(async (req, res, next) => {
  ...
  // Init chaincode service
  example02Service = new ChaincodeExample02Service(fabricStarterClient, CHANNEL_ID, CHAINCODE_ID)
...
const appRouter = (app) => {
  ...
  app.get('/', (req, res) => {
    res.status(200).send('Welcome to fabric-api-service REST server');
  });
  ...
  // Move
  app.post('/move', asyncMiddleware(async (req, res, next) => {
    let payload = req.body
    let {result, error} = await example02Service.move(payload)
  ...
  app.post('/delete', asyncMiddleware(async (req, res, next) => {
    let payload = req.body
    let {result, error} = await example02Service.delete(payload)
  ...
  // Query
  app.get('/query/:payload', asyncMiddleware(async (req, res, next) => { 
    let {result, error} = await example02Service.query_(req.params.payload)
...
const server = app.listen(process.env.PORT || 3000, function () {
    logger.info('started fabric-api-service server on port', server.address().port);
});

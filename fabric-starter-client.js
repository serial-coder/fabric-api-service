...
class FabricStarterClient {
    ...
    async retryInvoke(nTimes, resolve, reject, errMsg, fn) {
        ...
        try {
            ...
            // // Non-blocking and asynchronous (with delay)
            // setTimeout(() => {this.retryInvoke(--nTimes, resolve, reject, errMsg, fn)}, 3000);

            // Non-blocking and asynchronous (without delay, just wait until 
            // the event loop will come back to process this statement)
            setTimeout(() => {this.retryInvoke(--nTimes, resolve, reject, errMsg, fn)}, 0);

            // // Blocking and asynchronous (this should not be used)
            // this.retryInvoke(--nTimes, resolve, reject, errMsg, fn)
        }
    }
